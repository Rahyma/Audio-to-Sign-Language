...
def main():
   ...
   # for connection from database
    ...
    # for audio as a input from user
    try:
        with mic as source:
            print("Listening.....")
    ...
    for x in range(0, len(abc)):
        if abc[x] == " ":
            cursor.execute("select sign from sign where letter = ' ' ")
        else:
            cursor.execute("select sign from sign where letter = %s", (abc[x],))
            print(abc[x])
        # cursor.execute("select sign from sign where letter = %s", (abc[x],))
        # all leter images are retrive and save in data2
        data2 = cursor.fetchall()
        file_like2 = io.BytesIO(data2[0][0])
        ...
        # for animation stored those images that was retreive in storedimages folder.
...
# for animation:
    ...
    while running:
        ...
        dt = clock.tick(FPS) / 10000  # Amount of seconds between each loop.
        ...
        i = 1
        for file in os.listdir(r'C:\Users\MRC\Desktop\storedimages'):
            file = r'C:\Users\MRC\Desktop\storedimages\image' + str(i) + '.jpg'
            # print(i)
